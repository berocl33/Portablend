...
public class GhostActivity extends Activity implements SensorEventListener {
	...
	startconf= data != null ? data.getPath().toString():"/data/user/0/org.blender.play/conf.start";
   try{(new ProcessBuilder(new String[]{"chmod","777","/data/user/0/org.blender.play"})).start();
   }catch(Exception e){}
        mainSurface = new GhostSurface( "/data/user/0/org.blender.play/start.blend", getApplication());
...
class GhostSurface extends SurfaceView implements SurfaceHolder.Callback {
   ...
   //protected void onWindowVisibilityChanged(int visible){}
